...
def RedisToMongo():
    try: 
        ...
        #print("From Redis: $%s for time %s equal to %s BTC with hash `%s´" % (str(df.iloc[indx]['Amount (USD)']),str(df.iloc[indx]['Time']),str(df.iloc[indx]['Amount (BTC)']),str(df.iloc[indx]['Hash'])))  
        ...
        return "Hash added to mongo: %s for time %s (Zulu)" % (Hash,Time)
    
    except:
        return "Failed to contact MongoDB"
